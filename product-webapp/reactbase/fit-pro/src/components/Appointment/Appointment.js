...
function Appointment() {
  ...
  const FetchFunc = () =>
    fetch(`http://18.219.97.106:8080/appointmentservice/api/slot/expertslot/${emailId}`)
      .then((res) => res.json())
      .then((response) => {
        setData(response)
      })
  ...
  const handleAdd = (e) => {
    ...
    const res = axios.post("http://18.219.97.106:8080/appointmentservice/api/slot/newslot", {
  ...
  const handleCancel = (e) => {
    const deleteSlot = axios.delete(`http://18.219.97.106:8080/appointmentservice/api/slot/delete/${e}`
    ).then(res => {
      FetchFunc()
    })
    .catch(err => {});
  ...
  const handleSubmit = (e) => {
    ...
    //   }
    // }
    const res = axios.put(`http://18.219.97.106:8080/appointservice/api/slot/update`, updateData, {
      headers: {
